...
# # Page config: wide layout so elements can expand to full width
# st.set_page_config(page_title="Earthquake Analysis", layout="wide")

# # Load earthquake data and parse datetime columns so time-based queries work
# df = pd.read_csv("data/earthquakes_cleaned.csv", parse_dates=["time", "updated"], infer_datetime_format=True)
# df['time'] = pd.to_datetime(df['time'], errors='coerce')
# df['updated'] = pd.to_datetime(df['updated'], errors='coerce')

# # Small CSS to ensure Streamlit main container uses full width and items fit
# st.markdown(
#     """
#     <style>
...
# st.markdown("Select any problem statement (1-30) to run the corresponding SQL query.")

# # Define SQL-like queries (as small pandas lambdas)
# queries = {
#     "1. Earthquakes with magnitude > 5": lambda df: df[df['mag'] > 5],
#     "2. Top 10 deepest earthquakes (depth_km)": lambda df: df.nlargest(10, 'depth_km'),
#     "3. Average magnitude by region (latitude bands)": lambda df: df.groupby(pd.cut(df['latitude'], bins=10))['mag'].mean(),
#     "4. Earthquakes in last 30 days": lambda df: df[df['time'] > pd.Timestamp.now() - pd.Timedelta(days=30)],
#     "5. Strongest earthquakes (top 10 magnitude)": lambda df: df.nlargest(10, 'mag'),
#     "6. Earthquakes by month": lambda df: df.groupby(df['time'].dt.to_period('M')).size(),
#     "7. Count by magnitude ranges": lambda df: df.groupby(pd.cut(df['mag'], bins=[0, 3, 4, 5, 6, 10]))['id'].count(),
#     "8. Most active location (latitude)": lambda df: df['latitude'].mode(),
#     "9. Depth statistics": lambda df: df['depth_km'].describe(),
#     "10. Time range of data": lambda df: pd.DataFrame({"First": [df['time'].min()], "Last": [df['time'].max()]}),
# }

# # Task selector with Run button placed below the dropdown for vertical stacking
...
#         index=1,
#         help="Pick a pre-defined query to run on the dataset"
#     )
#     # Place the Run button below the dropdown, occupying 25% width
#     btn_left, btn_right = st.columns([1, 3])
#     with btn_left:
#         run_button = st.button("Run Query", use_container_width=True)

# # Run query and show results in a responsive table
# if run_button:
#     st.markdown(f"### Results for: {selected_task}")
#     query_func = queries[selected_task]
#     result = query_func(df)

#     if isinstance(result, pd.DataFrame):
#         st.dataframe(result, use_container_width=True, height=520)
#     elif isinstance(result, pd.Series):
#         st.dataframe(result.to_frame(), use_container_width=True, height=520)
#     else:
#         st.write(result)
...
queries = {
        ...
        SELECT DATENAME(WEEKDAY, time) AS day_name, COUNT(*) AS total_earthquakes
        FROM earthquakes_data
        ...
        SELECT DATEPART(HOUR, time) AS hour_of_day, COUNT(*) AS total_earthquakes
        FROM earthquakes_data
        ...
        SELECT TOP 5 place, SUM(casualties) AS total_casualties
        FROM earthquakes_data
        GROUP BY place
        ...
        SELECT TOP 5 country, AVG(mag) AS avg_magnitude
        FROM earthquakes_data
        WHERE time >= DATEADD(YEAR, -10, GETDATE())
        GROUP BY country
        ORDER BY avg_magnitude DESC
        """,

        "22. Countries with both shallow and deep earthquakes in same month": """
        SELECT DISTINCT country, YEAR(time) AS year, MONTH(time) AS month
        FROM earthquakes_data
        GROUP BY country, YEAR(time), MONTH(time)
        HAVING
            MAX(CASE WHEN depth_km < 70 THEN 1 ELSE 0 END) = 1
        AND MAX(CASE WHEN depth_km > 300 THEN 1 ELSE 0 END) = 1
        """,
        ...
        SELECT country,
            SUM(CASE WHEN depth_km < 70 THEN 1 ELSE 0 END) * 1.0 /
